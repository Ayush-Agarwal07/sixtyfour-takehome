...
| **Frontier′** | EXPAND frontier is a plain relevance-sorted list; the **planner ranks**. The multiplicative `p×slot×prior/cost` formula and the `class_prior`/cost tables are kept, not dropped — computed each batch as the formula-top baseline logged in `planner_decision`; the planner's picks are the decision | simplifies §5.2 / Stage 3 (ponytail) | Two stacked ranking systems for one decision; the formula was the least-agentic part and needed calibration nobody has time for. No deliverable changes; net-positive on Agentic Behavior. |
| **C6′ superseded** | Contested-identity quarantine dropped: `confirmed_contested` status and `identity_contested` flag were never produced by the build; removed | supersedes C6′ | A downstream contradiction on an immutable predicate is a plain soft `conflicts[]` entry, same as any other cross-source conflict — no distinct status, no flag nobody set. |
| **C13 superseded** | `inferences[]` removed from the output envelope | supersedes C13 | Nothing is stated without a claim: every synthesis sentence cites a claim id (T2 rule 1), so an unscored inference bucket never had a producer to fill it. |
